...
namespace Dia02
{
	...
	public class ContactsAdapter : BaseAdapter
	{
		...
		{          
			var view = convertView ?? _activity.LayoutInflater.Inflate (Resource.Layout.FormatoListagem, parent, false);
			...
			if (cursor.MoveToFirst ()) {
				do {
					_contactList.Add (new Contact{
						...
						Number = cursor.GetString (cursor.GetColumnIndex (projection [3])) ?? "- nenhum -"
					});
