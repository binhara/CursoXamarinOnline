	...
	public class TaskDatabase 
	{
		...
		public TaskDatabase (string dbPath) 
		{
			...
			if (!exists) {
				...
				connection.Open ();
				var commands = new[] {
					"CREATE TABLE [Items] (_id INTEGER PRIMARY KEY ASC, Name NTEXT, Email NTEXT, PhoneNumber NTEXT, Notes NTEXT, Done INTEGER);"
				};
		...
		Task FromReader (SqliteDataReader r) {
			...
			t.PhoneNumber = r ["PhoneNumber"].ToString ();
		...
		public IEnumerable<Task> GetItems ()
		{
			...
			lock (locker) {
				...
				using (var contents = connection.CreateCommand ()) {
					contents.CommandText = "SELECT [_id], [Name], [Email], [PhoneNumber], [Notes], [Done] from [Items]";
		...
		public Task GetItem (int id) 
		{
			...
			lock (locker) {
				...
				using (var command = connection.CreateCommand ()) {
					command.CommandText = "SELECT [_id], [Name], [Email], [PhoneNumber], [Notes], [Done] from [Items] WHERE [_id] = ?";
		...
		public int SaveItem (Task item) 
		{
			...
			lock (locker) {
				if (item.ID != 0) {
					...
					connection.Open ();
					using (var command = connection.CreateCommand ()) {
						command.CommandText = "UPDATE [Items] SET [Name] = ?,  [Email] = ?, [PhoneNumber] = ?, [Notes] = ?, [Done] = ? WHERE [_id] = ?;";
						...
						command.Parameters.Add (new SqliteParameter (DbType.String){ Value = item.PhoneNumber });
					...
					connection.Open ();
					using (var command = connection.CreateCommand ()) {
						command.CommandText = "INSERT INTO [Items] ([Name], [Email], [PhoneNumber], [Notes], [Done]) VALUES (? ,?, ?, ?, ?)";
						command.Parameters.Add (new SqliteParameter (DbType.String) { Value = item.Name });
						command.Parameters.Add (new SqliteParameter (DbType.String){ Value = item.Email });
						command.Parameters.Add (new SqliteParameter (DbType.String){ Value = item.PhoneNumber });
						command.Parameters.Add (new SqliteParameter (DbType.String) { Value = item.Notes });
